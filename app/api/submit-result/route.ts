/**
 * POST /api/submit-result   (MODULE 2: group matches only)
 *
 * Records one player's version of a group-stage result.
 *
 ...
 *
 * Module 3 will add the knockout branch (kind: 'knockout') to this same route.
 *
 * Body: SubmitGroupResultPayload
 * Response: SubmitResultResponse
 */

import { NextResponse } from 'next/server';
import type { GroupMatch, SubmitGroupResultPayload } from '@/types';
import { handleServerError, jsonError, readJsonBody } from '@/lib/api';
import { isSupabaseConfigured, supabaseAdmin } from '@/lib/supabase';
import { validateGroupResult } from '@/lib/validation';
...
export async function POST(request: Request) {
  try {
    ...
    // Module 3 turns this branch into the knockout flow.
    if (rawKind === 'knockout') {
      return jsonError(
